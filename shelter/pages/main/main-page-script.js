...
let Btns = document.querySelectorAll(".btn-o");
...
btns.forEach((el) => {
	el.addEventListener("click", function (e) {
		let petsId = e.target.closest(".card-wrapper").getAttribute("data-id");
		modalChilds[1].src = pets[petsId][`img`];
		modalContent[0].children[0].innerText = pets[petsId][`name`];
		modalContent[0].children[1].innerText =
			pets[petsId][`type`] + " - " + pets[petsId][`breed`];
		modalContent[0].children[2].innerText = pets[petsId][`description`];
		for (let c = 0; c < modalList.length; c++) {
			modalList[c].children[0].innerHTML = "<b>Age:</b> " + pets[petsId][`age`];
			modalList[c].children[1].innerHTML =
				"<b>Inoculations:</b> " + pets[petsId][`inoculations`];
			modalList[c].children[2].innerHTML =
				"<b>Diseases:</b> " + pets[petsId][`diseases`];
			modalList[c].children[3].innerHTML =
				"<b>Parasites:</b> " + pets[petsId][`parasites`];
		}
	});
});
